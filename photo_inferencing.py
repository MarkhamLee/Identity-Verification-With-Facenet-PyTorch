...
class inferencing:
    ...
    def __init__(self):
        ...
        self.mtcnn = MTCNN(160, 30, 20, [0.6, 0.7, 0.7], 0.709, True, True, None, False, device = self.device)

        # Instantiate Resnet for Facial Geometry (Embeddings)
        self.resnet = InceptionResnetV1(pretrained='vggface2', classify=True).eval().to(self.device)
